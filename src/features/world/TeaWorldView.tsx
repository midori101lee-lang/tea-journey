...
export default function TeaWorldView() {
  const { player, saveProgress, bootTo, enterRegion } = useGame();
  ...
  function handleEnter(e: React.MouseEvent<HTMLButtonElement>, r: TeaRegionCard) {
    ...
    setFly({ id: r.id, x: rect.left, y: rect.top, w: rect.width });
    const rid = r.regionId ?? r.id;
    window.setTimeout(() => {
      setFly(null);
      // 进入茶区：切换 currentRegion 并进入该茶区地图。
      // 首次从茶世界进入武夷山 → 先走老陈开场对话（intro）；已见过则直接进内部地图。
      if (rid === 'wuyishan') enterRegion('wuyishan', player.flags.saw_opening ? 'map' : 'intro');
      else enterRegion(rid, 'map');
    }, 680);
  }
  ...
  return (
    <div className="tw">
        ...
        {slice.map((r) => {
          ...
          return (
            <button
              key={r.id}
              type="button"
              className={`tw-card ${unlocked ? '' : 'locked'} ${fly?.id === r.id ? 'is-leaving' : ''}`}
              disabled={!unlocked}
              onClick={(e) => unlocked && handleEnter(e, r)}
            >
              <div className="tw-card-img">
                ...
                />
                {!unlocked && <span className="tw-veil" aria-hidden />}
              </div>
      ...
      <p className="tw-foot-hint">武夷山、杭州，都已在你脚下。</p>
